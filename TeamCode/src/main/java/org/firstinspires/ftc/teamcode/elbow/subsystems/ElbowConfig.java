package org.firstinspires.ftc.teamcode.elbow.subsystems;

import com.acmerobotics.dashboard.config.Config;

@Config
public class ElbowConfig {

    public static double ELBOW_KP = 0.15;
    public static double ELBOW_KI = 0.0;
    public static double ELBOW_KD = 0.004;
    public static double ELBOW_EXTENDED_INTAKE = 3.75; //  degrees
    public static double ELBOW_CLOSE_INTAKE = 1.0; //   degrees
    public static double ELBOW_DEPOSIT = 120.0; //  degrees
    public static double ELBOW_TRAVEL = 25.0; //  degrees
    public static double ELBOW_MAX = 160.0; //  degrees
    public static double ELBOW_MIN = 0.0; //  degrees


}
