...
@Autonomous
class RedStatesStackSide : AnchorOpMode() {
...
//        val startPose = Pose2d(-32.0, 62.0, startHeading).adjustForAlliance(alliance)
        val awayFromWallPosition = Vector2d(-42.0, -48.05)
        ...
        val purplePixelPose = when (zoneDetected) {
            PropZoneDetected.RIGHT -> if (alliance == Alliance.BLUE) purplePixelPoseBackdropSide else purplePixelPoseAwayFromBackdrop
            PropZoneDetected.CENTER, PropZoneDetected.NONE -> purplePixelPoseCenter
            PropZoneDetected.LEFT -> if (alliance == Alliance.BLUE) purplePixelPoseAwayFromBackdrop else purplePixelPoseBackdropSide
        }
...
//            Vector2d(nearBackDropLaneX, nearBackDropCenter.y - backDropZoneSpacing)
        ...
        val moveToCloseTransitLaneTrajectory =
            drive.trajectoryBuilder(moveToStackTrajectory.end())
                .lineToConstantHeading(transitLanePoseAfterPurplePixel)
                .build()
        ...
        val moveToBackdropTrajectory =
            drive.trajectoryBuilder(moveToCloseTransitLaneTrajectory.end(), true)
        ...
        val backAwayFromBackDropTrajectory =
            drive.trajectoryBuilder(moveToScoreBackDropTrajectory.end())
                .lineTo(transitLaneBackDropSide) // front is facing away from BB, possibly add a velocity/acceleration constraint here as we might ram the BB
                .build()
